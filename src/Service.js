...
export default class Service {
  ...
  async getDataSystem(url, path) {
    console.log(url, path);
    url = 'http://request-tst.sibgenco.local/corpsystems/';
    path = '/sap_devform/'
    return await this.getResource(`http://services/?data=system&url=${url}&path=${path}&key=N7Ej1YO2kqFH2FnqNiKA6tm980bwMS`);
    // return await this.getResource(`https://request-tst.sibgenco.local/services/?data=system&url=${url}&path=${path}&key=N7Ej1YO2kqFH2FnqNiKA6tm980bwMS`);
